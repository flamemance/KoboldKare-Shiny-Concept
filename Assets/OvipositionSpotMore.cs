...
public class OvipositionSpotMore : GenericUsable, IAnimationStationSet {
    public delegate void OvipositionAction(int koboldID, int eggID, int bronzeeggID);
    ...
    [SerializeField]
    private PhotonGameObjectReference eggPrefab, bronzeeggPrefab;

    private ScriptableReagent egg, bronzeegg;
    ...
    private bool KoboldReadyToLayEgg(Kobold k){
        ...
        return k.bellyContainer.GetVolumeOf(bronzeegg) > 5f && k.GetEnergy() >= 1f;
    }
    ...
    void Start() {
        ...
        bronzeegg = ReagentDatabase.GetReagent("BronzeEgg");
    }
    ...
    IEnumerator EggLayingRoutine() {
        ...
        eggVolume = k.bellyContainer.GetVolumeOf(bronzeegg);
        k.bellyContainer.OverrideReagent(egg, 0f);
        k.bellyContainer.OverrideReagent(bronzeegg, 0f);
        ...
        ReagentContents bronzeeggContents = new ReagentContents();
        bronzeeggContents.AddMix(ReagentDatabase.GetReagent("ScrambledEggBronze").GetReagent(eggVolume));
        
        buffer.AddReagentContents(eggContents);
        PhotonView bronzeeggView = d.gameObject.GetPhotonView();
        bronzeeggView.RPC(nameof(GenericReagentContainer.ForceMixRPC), RpcTarget.All, buffer, k.photonView.ViewID, (byte)GenericReagentContainer.InjectType.Inject);

        oviposition?.Invoke(k.photonView.ViewID, eggView.ViewID, bronzeeggView.ViewID);
